...
import { BiCheck, BiQuestionMark, BiTime, BiX } from "react-icons/bi";
        ...
        <ul>
            {violationLogResults.map(({ traceId, trace, results }) => (
                ...
                >
                    <Label>{traceId}</Label>
                    <ResultContainer>
                        {results !== undefined ? results.totalViolations - results.totalTimeViolations : undefined}
                        {resultIcon(results?.totalViolations === 0)}
                        {results !== undefined && (
                            <>
                                {results.totalTimeViolations}
                                <BiTime title="Time Violations" style={{ backgroundColor: results.totalTimeViolations > 0 ? '#FFD600' : '#adb5bd' }} />
                            </>
                        )}
                    </ResultContainer>
                </ResultsElement>))}
