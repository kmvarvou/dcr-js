...
const ModelerState = ({ setState, savedGraphs, setSavedGraphs, lastSavedGraph }: StateProps) => {
  ...
  // Don't pass our new format XML to the old modeler — it can't parse it
  const lastXml = lastGraph ? savedGraphs[lastGraph] : undefined;
  const isNewFormat = lastXml && !lastXml.includes('dcrDi:') && lastXml.includes('<dcr:definitions');
  const initXml = isNewFormat ? undefined : lastXml;
