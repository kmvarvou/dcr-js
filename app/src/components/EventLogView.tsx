...
const EventLogView = ({ children, selectedTrace, eventLog, setSelectedTrace, traceRef, editProps, defaultVariables }: EventLogViewProps) => {
    const defaultEntries = defaultVariables ? Object.entries(defaultVariables) : [];
    return (
        <ResultsWindow $traceSelected={selectedTrace !== null}>
            ...
            {defaultEntries.length > 0 && (
                <div style={{ fontSize: 13, color: '#6c757d', padding: '0.25rem 1rem', borderBottom: '1px solid #dee2e6' }}>
                    <strong>Defaults:</strong> {defaultEntries.map(([k, v]) => `${k} = ${v}`).join(' · ')}
                </div>
            )}
