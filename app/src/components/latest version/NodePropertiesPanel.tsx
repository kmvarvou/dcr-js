...
export type VariableType = 'Int' | 'Bool' | 'void';
...
const NodePropertiesPanel = ({
    ...
    const addVariable = () => {
        const vars: DCRVariable[] = data.variables || [];
        onUpdateNode(selectedNode.id, {
            variables: [...vars, { id: `var_${Date.now()}`, name: '', type: 'Int' }]
        });
    };

    const updateVariable = (varId: string, changes: Partial<DCRVariable>) => {
        const vars: DCRVariable[] = data.variables || [];
        onUpdateNode(selectedNode.id, {
            variables: vars.map(v => v.id === varId ? { ...v, ...changes } : v)
        });
    };

    const removeVariable = (varId: string) => {
        const vars: DCRVariable[] = data.variables || [];
        onUpdateNode(selectedNode.id, { variables: vars.filter(v => v.id !== varId) });
    };
    ...
    return (
        <Panel $visible={true}>
            <PanelHeader>
                ...
                <NodeLabel>{data.label || selectedNode.id}</NodeLabel>
            </PanelHeader>
            ...
            <PanelBody>
                ...
                <Section>
                    ...
                    <VarList>
                        {(data.variables || []).map((v: DCRVariable) => (
                            <VarRow key={v.id}>
                                <VarInput
                                    value={v.name}
                                    placeholder="name"
                                ...
                                <VarSelect
                                    value={v.type}
                                    onChange={e => updateVariable(v.id, { type: e.target.value as VariableType })}
                                    onMouseDown={e => e.stopPropagation()}
                                >
                                ...
                                <RemoveBtn onClick={() => removeVariable(v.id)}
                                    onMouseDown={e => e.stopPropagation()} title="Remove">×</RemoveBtn>
                            </VarRow>
                        ))}
                    </VarList>
                    <AddVarButton onClick={addVariable} onMouseDown={e => e.stopPropagation()}>
                        + Add Variable
                    </AddVarButton>
                </Section>
            </PanelBody>
