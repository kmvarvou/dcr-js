...
/**
 * Parse and render a Dcr-diagram.
 *
 ...
 * @param {ModdleElement<DCRRootBoard>|String} [rootBoard] Postit board or id of board to render (if not provided, the first one will be rendered)
 *
 ...
 * You can use these events to hook into the life-cycle.
 *
 * @param {String|ModdleElement<DcrRootBoard>} [rootBoardOrId] id or the diagram to open
 *
...
BaseViewer.prototype.open = function (rootBoardOrId) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof rootBoardOrId === 'string') {
      ...
      if (!rootBord) {
        var err2 = new Error('DcrRootBoard <' + rootBoardOrId + '> not found');
    ...
    importDcrDiagram(self, definitions, rootBord).then(function (result) {
...
/**
 * Export the currently displayed dcr diagram as
 * a dcr XML document.
 *
 * ## Life-Cycle Events
 *
...
BaseViewer.prototype.saveDCRXML = function (options) {
  return saveDCRXML(options, this._definitions);
};
...
/**
 * Export the currently displayed DCR graph diagram as
 * an SVG image.
 *
 * ## Life-Cycle Events
 *
...
function checkValidationError(err) {
  ...
  if (match) {
    err.message =
      ...
      'this may indicate an invalid Dcr board file' + match[2];
  }
